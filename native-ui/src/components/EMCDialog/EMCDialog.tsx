...
const EMCDialog = ({
  ...
  function confirmRequest() {
    if (requestData.trim().length === 0) {
      showToast(
        alertMessageEmptyResponse ?? "Por favor, informe os dados solicitados",
        "alert",
        undefined,
        insets.top
      );
  ...
  return (
    <Modal transparent={true} animationType="fade" {...rest}>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <EMCVStack flex={1}>
          <EMCVStack flex={1} justify="center" px={isTablet ? "20%" : 6}>
            <EMCVStack
              bg={Colors.light[50]}
              rounded="md"
              p={4}
            ...
            >
              {!noHeader && (
                ...
                >
                  ...
                  />
                  <EMCText mb={0} fSize={isTablet ? "lg" : "md"} fWeight="bold">
                    {dialogType === "alert"
                      ? "Atenção!"
                      : dialogType === "error"
                      ? "Ops!"
              ...
              {handleConfirm || handleConfirmRequest ? (
                <EMCHStack align="center" justify="center">
                  <EMCButton
                    mr={3}
                    isDialog
                    variant="outline"

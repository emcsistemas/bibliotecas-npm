...
export function editFloatString(textValue?: string) {
  if (textValue) {
    textValue = textValue
      .replace('R$', '')
      .replace('%', '')
...
export function extractCurrencyValue(currencyText: string) {
...
export function formatDecimalValue(decimalText: string, putSymbol: boolean) {
...
export function truncValue(value: number) {
  if (value > 0) {
...
export function roundValue(value: number, decimalPlaces: number) {
  return parseFloat(value.toFixed(decimalPlaces))
}

export function roundValueStr(value: number, decimalPlaces: number) {
  const roundedValue = roundValue(value, decimalPlaces)
